...
typedef struct VVCIntraDSPContext {
    ...
    void (*palette_pred)(struct VVCLocalContext *lc, int start_comp, int num_comp, const uint8_t *hs, const uint8_t *vs, const int *qp);
} VVCIntraDSPContext;
...
typedef struct VVCItxDSPContext {
    ...
    void (*pred_residual_joint)(int *dst, int *src, int width, int height, int c_sign, int shift);
    ...
    void (*adaptive_color_transform)(int *res_y, int *res_cb, int *res_cr, int width, int height);
} VVCItxDSPContext;
