...
{
    // Don't reset the film grain characteristics as it may be used by the subsequent frame
    // See H.274 8.5.2 Film grain characteristics SEI message semantics - fg_characteristics_cancel_flag
    H2645SEIFilmGrainCharacteristics *fgc = s->common.film_grain_characteristics;
    s->common.film_grain_characteristics = NULL;

    ff_h2645_sei_reset(&s->common);
    s->common.film_grain_characteristics = fgc;
