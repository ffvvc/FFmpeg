                     ...
                     H266RawSPS *current)
{
    ...
    {
        current->sub_width_c = h266_sub_width_c[current->sps_chroma_format_idc];
        current->sub_height_c = h266_sub_height_c[current->sps_chroma_format_idc];
        current->min_cb_size_y = 1 << (current->sps_log2_min_luma_coding_block_size_minus2 + 2);
    }
                     ...
                     H266RawPPS *current)
{
    ...
    current->min_cb_width  = current->pps_pic_width_in_luma_samples  / sps->min_cb_size_y;
    current->min_cb_height = current->pps_pic_height_in_luma_samples / sps->min_cb_size_y;
    current->log2_ctb_size = current->pps_log2_ctu_size_minus5 + 5;
    current->ctb_width  = current->pps_pic_width_in_luma_samples  / ctb_size_y;
    current->ctb_height = current->pps_pic_height_in_luma_samples / ctb_size_y;
                              ...
                              H266RawSliceHeader *current)
{
    ...
    {
        int min_cb_log2_size_y = sps->sps_log2_min_luma_coding_block_size_minus2 + 2;
        int min_qt_log2_size_y, min_qt_log2_size_c;
        if (current->sh_slice_type == VVC_SLICE_TYPE_I) {
            min_qt_log2_size_y = min_cb_log2_size_y +
                ph->ph_log2_diff_min_qt_min_cb_intra_slice_luma;
            min_qt_log2_size_c = min_cb_log2_size_y +
                ph->ph_log2_diff_min_qt_min_cb_intra_slice_chroma;
            current->max_bt_size_y = 1 <<
                (min_qt_log2_size_y + ph->ph_log2_diff_max_bt_min_qt_intra_slice_luma);
            current->max_bt_size_c = 1 <<
                (min_qt_log2_size_c + ph->ph_log2_diff_max_bt_min_qt_intra_slice_chroma);
            current->max_tt_size_y = 1 <<
                (min_qt_log2_size_y + ph->ph_log2_diff_max_tt_min_qt_intra_slice_luma);
            current->max_tt_size_c = 1 <<
                (min_qt_log2_size_c + ph->ph_log2_diff_max_tt_min_qt_intra_slice_chroma);
            current->max_mtt_depth_y = ph->ph_max_mtt_hierarchy_depth_intra_slice_luma;
            current->max_mtt_depth_c = ph->ph_max_mtt_hierarchy_depth_intra_slice_chroma;
        } else {
            min_qt_log2_size_y = min_cb_log2_size_y +
            ...
            min_qt_log2_size_c = min_cb_log2_size_y +
                ph->ph_log2_diff_min_qt_min_cb_inter_slice;
            current->max_bt_size_y = 1 <<
                (min_qt_log2_size_y + ph->ph_log2_diff_max_bt_min_qt_inter_slice);
            current->max_bt_size_c = 1 <<
                (min_qt_log2_size_c + ph->ph_log2_diff_max_bt_min_qt_inter_slice);
            current->max_tt_size_y = 1 <<
                (min_qt_log2_size_y + ph->ph_log2_diff_max_tt_min_qt_inter_slice);
            current->max_tt_size_c = 1 <<
                (min_qt_log2_size_c + ph->ph_log2_diff_max_tt_min_qt_inter_slice);
            current->max_mtt_depth_y = ph->ph_max_mtt_hierarchy_depth_inter_slice;
            current->max_mtt_depth_c = ph->ph_max_mtt_hierarchy_depth_inter_slice;
        }
        current->min_qt_size_y = 1 << min_qt_log2_size_y;
        current->min_qt_size_c = 1 << min_qt_log2_size_c;
    }
