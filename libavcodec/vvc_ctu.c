...
static enum IntraPredMode luma_intra_pred_mode(VVCLocalContext* lc, const int intra_subpartitions_mode_flag)
{
    ...
    ff_vvc_set_cb_tab(lc, fc->tab.ipm, pred);
    return pred;
}
...
static void intra_luma_pred_modes(VVCLocalContext *lc, const int cu_act_enabled_flag)
{
    ...
    if (cu->intra_mip_flag) {
        ...
        for (int y = 0; y < (cb_height>>log2_min_cb_size); y++) {
            ...
            memset(&fc->tab.ipm[x], intra_mip_mode, width);
...
static void intra_chroma_pred_modes(VVCLocalContext *lc, int x0, int y0)
{
    const int cclm_enabled = get_cclm_enabled(lc, x0, y0);
    ...
    if (cu->pred_mode == MODE_INTRA || cu->pred_mode == MODE_PLT) {
        ...
        if ((tree_type == SINGLE_TREE || tree_type == DUAL_TREE_CHROMA) && sps->chroma_format_idc) {
            if (pred_mode_plt_flag && tree_type == DUAL_TREE_CHROMA)
                ...
                if (!cu->act_enabled_flag) {
                    if ((cb_width  >> hs) <= sps->max_ts_size &&
                        (cb_height >> vs) <= sps->max_ts_size &&
                        sps->bdpcm_enabled_flag) {
                        av_assert0(0 && "fix me");
                        //cu->bdpcm_flag[1] = cu->bdpcm_flag[2] =
                    }
                    if (cu->bdpcm_flag[1])
                        av_assert0(0 && "fix me");
                    else {
                        intra_chroma_pred_modes(lc, x0, y0);
                    }
